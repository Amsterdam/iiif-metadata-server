...
class BouwDossier(BouwDossierBase):
    ...
    importfile = models.ForeignKey(
        ImportFile, related_name="bagimporter_bouwdossiers", on_delete=CASCADE
    )
...
class Adres(AdresBase):
    ...
    bouwdossier = models.ForeignKey(
        BouwDossier, related_name="bagimporter_adressen", on_delete=CASCADE
    )
...
class Document(DocumentBase):
    id = models.AutoField(primary_key=True)
    bouwdossier = models.ForeignKey(
        BouwDossier, related_name="bagimporter_documenten", on_delete=CASCADE
    )
